...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                ...
                newmsg = 'Kiyu is now playing: **%s**' % (entry.title)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            reply_text = "Kiyu added **%s** songs to be played. Position in queue: %s"
            ...
            reply_text = "Kiyu added **%s** to be played. Position in queue: %s"
            btext = entry.title
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        if not info:
            return Response("Kiyu didn't find anything.", delete_after=30)
        ...
        for e in info['entries']:
            ...
            confirm_message = await self.safe_send_message(channel, "Is this okay? Type `y`, `n` or `exit`")
            response_message = await self.wait_for_message(30, author=author, channel=channel, check=check)

            if not response_message:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return Response("Okay, nevermind.", delete_after=30)
    ...
    async def cmd_replay(self, player, channel, permissions, server, message):
        """
        Usage:
            {command_prefix}replay

        Queues the current song again once.
        """
        #async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if player.current_entry:
            ...
            entry_list, position = await player.playlist.import_from(player.current_entry.title, channel=channel, author=author)
            
            reply_text = "Kiyu will replay **%s**. Position in queue: %s"
            btext = player.current_entry.title
